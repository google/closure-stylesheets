...
/**
 * Compact-printer for {@link CssTree} instances that only outputs rulesets which include a selector
 * belonging to one chunk. This printer does not support code moving between chunks and ouputs the
 * same ruleset for as many chunks as this ruleset belongs to.
 *
 ...
 */
public class ChunkCompactPrintingVisitor<T> extends CompactPrintingVisitor {

  /** Chunk to be printed by this printer. */
  private final T chunk;

  /**
   * Whether currently visited selector (including it's children) belongs to printed chunk and
   * should be printed.
   */
  private boolean printSelector;

  /** Create a chunk printer for a given chunk. */
  public ChunkCompactPrintingVisitor(
      VisitController visitController, T chunk, @Nullable CodeBuffer buffer) {
