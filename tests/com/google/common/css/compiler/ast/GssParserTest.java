...
public class GssParserTest {
  ...
  public void testAst15() throws Exception {
    testTree("@component a { x {y: z} }", "[@component [a]{[x]{[y:[[z]];]}}]");
  }
  ...
  public void testParsingLocalFunctions() throws Exception {
    testValid(
        "@font-face { src: local(Gentium), url(Gentium.woff);"
        +
  ...
  public void testMicrosoftListAtRule() throws Exception {
    ...
    // the whole parse.
    String[] samples = new String[] {"@list l0\n"
  ...
  public void testRunawayMicrosoftListAtRule() throws Exception {
    ...
    for (String css : samples) {
      try {
        parse(css);
        Assert.fail(
            "The compiler should only accept complete @list rules, not " + css);
      } catch (GssParserException e) {
        // expected
      }
    }
  ...
  public void testCustomBorderProperty() throws Exception {
    testTree("a { border-height: 1em; }", "[[a]{[border-height:[[1em]];]}]");
    testTree("a { border-left-height: 1em; }",
             "[[a]{[border-left-height:[[1em]];]}]");
    testTree("a { border-right-height: 1em; }",
             "[[a]{[border-right-height:[[1em]];]}]");
    testTree("a { border-top-height: 1em; }",
             "[[a]{[border-top-height:[[1em]];]}]");
    testTree("a { border-bottom-height: 1em; }",
             "[[a]{[border-bottom-height:[[1em]];]}]");
  }

  @Test
  public void testForLoop() throws Exception {
    testTree("@for $i from 1 to 6 {}", "[@for [$i] [from] [1] [to] [6]{}]");
  }

  @Test
  public void testForLoopWithStep() throws Exception {
    testTree("@for $i from 1 to 6 step 2 {}",
             "[@for [$i] [from] [1] [to] [6] [step] [2]{}]");
  }

  @Test
  public void testForLoopWithVariables() throws Exception {
    testTree("@for $i from $x to $y step $z {}",
             "[@for [$i] [from] [$x] [to] [$y] [step] [$z]{}]");
  }

  @Test
  public void testForLoopWithVariablesInBlock() throws Exception {
    testTree("@for $i from 1 to 2 { .foo-$i { padding: $i } }",
  ...
  public void testCalc_withParenthesizedSums() throws Exception {
    ...
    testTree("p { width: calc(4 * (5px * 2)); }",
             "[[p]{[width:[calc([[4]*[([5px]*[2])]])];]}]");
  }

  @Test
  public void testCalc_fourOperands() throws Exception {
    testValid("p { width: calc(4 + 5 + 6 + 7);}");
    testTree("p { width: calc(4 + 5 + 6 + 7);}",
             "[[p]{[width:[calc([[4] + [[5] + [[6] + [7]]]])];]}]");
  }
  ...
  public void testNumericNodeLocation() throws GssParserException {
    CssTree tree =
        new GssParser(new SourceCode(null, "div{width:99px;}")).parse();
    ...
    assertThat(location.getEndCharacterIndex() -
  ...
  public void testVariables() throws Exception {
    testValid(":root { --main-color: #06c;} h1 { color: var(--main-color);}");
  }
